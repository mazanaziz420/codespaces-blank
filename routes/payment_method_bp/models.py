...
class PaymentMethod:
    def __init__(self, user_id, user_type, card_holder_name, card_number, card_expiry, card_cvv, card_type):
    ...
    @staticmethod
    def find_by_user_type(user_type):
        try:
            payment_methods = mongo.db['PaymentMethods'].find({"user_type": user_type})
            return [{**method, '_id': str(method['_id'])} for method in payment_methods]
        except Exception as e:
            print(str(e))
            return e
